...
import { getPublicHttpsClient } from "../viemClients";
import { TypedEvent, TypedEventFilter } from "./interfaces";
import { Contract } from "@wagmi/cli/config";

export type FetchedEvent<TEvent extends Log> = {
...
export class EventFetcher {
  public constructor(public readonly clientWallet: Client<CustomTransport>) {}
  ...
  >(
    ...
    const logs = await getPublicHttpsClient().getLogs(fullFilter);
    const lala = logs
      ...
      .map((l) => {
        ...
        return {
          ...
          topics: l.topics,
          data: l.data,
        } as unknown as FetchedEvent<Log>;
      });
    return lala;
  }
}
